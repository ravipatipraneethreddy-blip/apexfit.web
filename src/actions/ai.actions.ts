...
export async function analyzeFoodImage(base64Image: string) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert nutritionist AI. The user will provide an image of food. Identify the food, estimate the serving size based on an average plate, and return the nutritional macros strictly in this JSON format: { \"foodName\": \"...\", \"calories\": number, \"protein\": number, \"carbs\": number, \"fats\": number, \"fiber\": number }."
          },
          {
            role: "user",
            content: [
    ...
    return { error: err.message || "Failed to analyze image." };
  }
}
